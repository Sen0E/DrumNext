...
停止播放时，投影端会在 1.5 秒内渐入投影增强的待机动画：中心鼓面蓄能、
多层粗亮涟漪向外扩散，随后以随机起点、方向、速度和单双路线追光，
并伴随少量大型彗星拖尾；涟漪会舒缓展开，每轮追光会生成新的组合且不会连续重复。
暂停会保持当前演奏画面，不进入待机；结束动画完成后会自然过渡到待机画面。
