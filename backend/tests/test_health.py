...
async def test_fastapi_hosts_api_debug_page(tmp_path: Path) -> None:
    ...
    assert "覆盖全部 18 个业务 REST API" in response.text
    ...
    assert "GET /api/v1/settings/projection-visuals" in response.text
    assert "PUT /api/v1/settings/projection-visuals" in response.text
